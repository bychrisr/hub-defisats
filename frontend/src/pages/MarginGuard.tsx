...
import { Loader2, Shield, AlertTriangle, Settings, Play, Pause, Trash2 } from 'lucide-react';
...
export default function MarginGuard() {
  ...
  const handleDelete = async () => {
    if (marginGuardAutomation && confirm('Are you sure you want to delete this Margin Guard?')) {
      try {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Protect your positions from liquidation</p>
          </div>
          {marginGuardAutomation && (
            <div className="flex items-center space-x-2">
              <Badge variant={marginGuardAutomation.is_active ? 'default' : 'secondary'}>
              ...
              <CardContent>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="margin_threshold">Margin Threshold (%)</Label>
                    <Input
                      id="margin_threshold"
                      type="number"
                      step="0.1"
                      ...
                      className={errors.margin_threshold ? 'border-red-500' : ''}
                      disabled={!isEditing}
                    />
                    {errors.margin_threshold && (
                      <p className="text-sm text-red-500">{errors.margin_threshold.message}</p>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="reduce_position">Reduce Position</SelectItem>
                  ...
                  {selectedAction === 'reduce_position' && (
                    <div className="space-y-2">
                      <Label htmlFor="reduce_percentage">Reduce Percentage (%)</Label>
                      <Input
                        ...
                        className={errors.reduce_percentage ? 'border-red-500' : ''}
                        disabled={!isEditing}
                      />
                      {errors.reduce_percentage && (
                        <p className="text-sm text-red-500">{errors.reduce_percentage.message}</p>
                      )}
                    </div>
                  )}
                  ...
                  {selectedAction === 'add_margin' && (
                    <div className="space-y-2">
                      <Label htmlFor="add_margin_amount">Additional Margin (sats)</Label>
                      <Input
                        ...
                        className={errors.add_margin_amount ? 'border-red-500' : ''}
                        disabled={!isEditing}
                      />
                      {errors.add_margin_amount && (
                        <p className="text-sm text-red-500">{errors.add_margin_amount.message}</p>
                      )}
                    </div>
                  )}
                ...
                {marginGuardAutomation ? (
                  <>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm">{marginGuardAutomation.config.margin_threshold}%</span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm">
                        {new Date(marginGuardAutomation.created_at).toLocaleDateString()}
                      </span>
                    </div>
                  </>
                ) : (
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm text-gray-600">No Margin Guard configured</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Important:</strong> Margin Guard is a safety mechanism but cannot guarantee 
                protection against all market conditions. Always monitor your positions and maintain 
